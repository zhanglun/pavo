...
export function AppShell() {
  ...
  return <main aria-label="Pavo" role="application"><header><span data-tauri-drag-region>Pavo</span><button aria-label="刷新">↻</button><button aria-label="设置" onClick={() => setView("settings")}>⚙</button><button aria-label="隐藏到托盘" onClick={() => { void hideWindow(); }}>−</button></header><nav>{(["today", "history", "favorites"] as const).map((item) => <button key={item} role="tab" aria-selected={view === item} onClick={() => setView(item)}>{item === "today" ? "今日" : item === "history" ? "历史" : "收藏"}</button>)}</nav><section>{view === "settings" ? "设置" : view === "today" ? "今日" : view === "history" ? "历史" : "收藏"}</section></main>;
}
