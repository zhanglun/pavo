...
更新时间：2026-07-23
当前状态：React 重构与旧前端清理已完成，进入最终人工验收、Windows 实机验证和合并准备阶段
...
- 最新提交：`1cf1fc0 fix(ui): 明确 Bing 来源链接语义`
- 相对 `main`：24 个提交
...
- `pnpm build`：通过。
- `cargo check`：通过。
- Rust 测试：固定 fixture 后通过。
- `pnpm tauri dev`：macOS 可冷启动并显示真实数据。
- macOS 视觉冒烟：窗口背景、主图、地区差异、Toast、Tooltip 和细滚动条已检查。
...
1. 按验收清单完成 macOS 最终人工验收并记录结果。
2. 修复验收中发现的问题，每项单独验证和提交。
3. 完成 Windows 实机冒烟。
4. 更新截图和发布文档。
5. 执行前端构建、Rust 全量测试与双平台最终验收。
6. 合并 `feature/react-daily-folio` 或创建 PR。
